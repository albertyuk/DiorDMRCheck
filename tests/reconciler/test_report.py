...
def test_override_over_source_s_is_recorded(plog_path, dmr_path,
    ...
    # the trimmed layout has no NOTES column — the audit trail is recorded
    # in STATUS (first evidence column) instead
    status = ann.cell(row=row, column=20).value or ""
    assert "旧人工备注" in status and "replaced by override" in status
