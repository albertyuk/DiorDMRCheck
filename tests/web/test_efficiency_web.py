...
def test_invalid_config_values_fall_back_to_defaults(client):
    r = _upload(client, basis="nonsense", tier_mode="nonsense",
                fanbase_unit="nonsense", language="xx")
    assert r.status_code == 200
    assert "Basis: pooled" in r.text
    token = list(eff_routes.EFF_REPORTS)[-1]
    assert eff_routes.EFF_REPORTS[token]["analysis"]["config"]["fanbase_unit"] == "k"


def test_raw_fanbase_unit_is_applied_and_shown(client):
    form = client.get("/efficiency")
    assert 'name="fanbase_unit"' in form.text

    response = _upload(client, fanbase_unit="raw")
    assert response.status_code == 200
    token = list(eff_routes.EFF_REPORTS)[-1]
    analysis = eff_routes.EFF_REPORTS[token]["analysis"]
    assert analysis["config"]["fanbase_unit"] == "raw"
    assert any(f["code"] == "V13" for f in analysis["findings"])
    assert "FAN BASE unit" in response.text
    assert "V13: FAN BASE UNIT RAW FOLLOWERS" in response.text
