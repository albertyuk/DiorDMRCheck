...
ZH: dict[str, str] = {
    ...
    "One unit is applied to the entire workbook; values are never reinterpreted row by row.":
        "整份工作簿统一使用一个单位；绝不会逐行猜测或改换单位。",
    "Raw followers — 450,000 means 450K":
        "原始粉丝数——450,000 表示 450K",
    "Thousands (K) — 450 means 450K":
        "千位（K）——450 表示 450K",
    ...
    "TYPE values start with 报备 (paid) or 软植 (soft); LEVEL is 头部/腰部/尾部/底部/KOC. Blank LEVEL values and the approved placeholders 待定/? fall back to FAN BASE thresholds (reported as V12); other unknown labels remain unclassified. PRICE is in CNY. The source file's own CPM/CPE columns are not needed and never reused.":
        "TYPE 以 报备 或 软植 开头；LEVEL 为 头部/腰部/尾部/底部/KOC。LEVEL 为空或为已批准占位值「待定／?」时，按 FAN BASE 阈值回退分层（以 V12 提示）；其他未知标签仍保持未分类。PRICE 为人民币。源文件自带的 CPM/CPE 列不需要，也绝不会被复用。",
