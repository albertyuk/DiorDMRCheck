...
| `EFF_MAX_UPLOAD_MB` / `EFF_MAX_XLSX_UNCOMPRESSED_MB` / `EFF_MAX_SHEET_ROWS` | The efficiency workbook's own budgets; default 40 MB compressed / 1 GB expanded / 100,000 data rows. Its analyzer streams only the sheet it reads, so other tabs, styling, and pasted images cost nothing; the expanded cap is purely a ZIP-bomb ceiling and the row cap bounds actual work. |
